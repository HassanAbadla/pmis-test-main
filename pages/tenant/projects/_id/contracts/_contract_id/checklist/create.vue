...
<script>
import { createContractChecklist, getContractByIdAndProjectId } from "../../../../../../../api/projects-api"
...
export default {
  ...
  methods: {
    ...
    submitChecklist() {
      ...
      this.fields.contract_id = this.$route.params.contract_id
      createContractChecklist(this.fields, this.$axios)
        .then((response) => {
          this.$router.push(
            this.localePath(
              `/tenant/projects/${this.$route.params.id}/contracts/${this.$route.params.contract_id}/checklist`
            )
          )
        })
        .catch((error) => {
